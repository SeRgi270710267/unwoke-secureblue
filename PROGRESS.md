...
54. **Overlay now:** `apply-unwoke.sh` saves `/usr/share/unwoke/.rpmdb-pre-flavor.sqlite`. Origin `rebuild-rpmdb.sh` always restores that copy after Brave dnf, then deletes the bak (must not ship). WAL checkpoint only. `common.yml` ships `sqlite` CLI. Inspect checks no WAL. Overlay bake [90](https://github.com/SeRgi270710267/unwoke-secureblue/actions/runs/33298098294) (WAL-only) green. Restore bake [33302539888](https://github.com/SeRgi270710267/unwoke-secureblue/actions/runs/33302539888) (`6f22bd9`) green — compose log: `Origin — restoring pre-flavor rpmdb`. Live wrap then saw **95 MiB** sqlite, sysimage and `%_dbpath=/usr/share/rpm` the **same file**, but `rpm -q kernel-core` still failed until `--justdb`.
55. **ISO hook now** (`.github/workflows/isos/prep_initramfs.sh`, paths-ignore, no overlay bake): squashfs override; if `rpm -q kernel-core` already works, write `/etc/dnf/vars/releasever` and stop. If the sqlite is **≥20 MiB**, skip recover; `dnf5 download kernel-core-$krel` + `rpm --justdb --nodeps -ivh` so titanoboa’s queryformat works. Recover only a small/stub db. Do **not** `dnf install dracut-live` in the hook (that pulled 100+ pkgs). Live USB only.

**Next Origin USB test (do not cancel):** [iso run 33304494809](https://github.com/SeRgi270710267/unwoke-secureblue/actions/runs/33304494809) — `unwoke-silverblue` on `e7494a7` hook (justdb onto 95 MiB index, no recover). Wrap uses GHCR `:latest` from restore bake `6f22bd9`. If **green**: dispatch the other three Origin flavors (`unwoke-silverblue-nvidia-open`, `unwoke-kinoite`, `unwoke-kinoite-nvidia-open`). If **initramfs still red**: read hook lines `skipping recover on large rpmdb` / `justdb registered` / titanoboa `dnf install dracut-live` package count. A full index should install ~1 package, not 189.

**Next overlay (only if Origin USB still needs it):** add `.rpmdb-pre-flavor.sqlite` to `mark-check.py` skip list — Trivalent compose **stamped** that sqlite (`stamped usr/share/unwoke/.rpmdb-pre-flavor.sqlite`). Origin restore bak is created after `--apply` in the same script so it should be clean; still do not stamp sqlite. Optional: skip `selinux-policy-devel` dnf (prebuilt `.pp` / CIL) so Origin extra dnf stops malforming Packages in the first place.
...
- Close-chat HEAD after this file is pushed: `git log -1 --oneline`. Product overlay in GHCR `:latest` is restore bake `6f22bd9` / run [33302539888](https://github.com/SeRgi270710267/unwoke-secureblue/actions/runs/33302539888). Later iso-hook commits (`797ad85`, `7a3125a`, `e7494a7`) are **docs/iso-only** (paths-ignore).
- **In flight — do not cancel:** Origin USB [33304494809](https://github.com/SeRgi270710267/unwoke-secureblue/actions/runs/33304494809) (`unwoke-silverblue`). `cancel-in-progress: false`.
- USB: **8 green** (all Trivalent + all browserless). Origin still red until 33304494809 (or a follow-up) is green.
