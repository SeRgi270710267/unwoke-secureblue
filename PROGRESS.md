...
54. **Overlay now:** `apply-unwoke.sh` saves `/usr/share/unwoke/.rpmdb-pre-flavor.sqlite` **plus WAL/SHM**. Origin `rebuild-rpmdb.sh` drops live sidecars first, restores that set after Brave dnf, then deletes the bak (must not ship). Do **not** copy sqlite onto Origin’s leftover WAL — that is what malformed the 95 MiB index. WAL checkpoint only after the set matches. `common.yml` ships `sqlite` CLI. Inspect checks no WAL. Overlay bake [90](https://github.com/SeRgi270710267/unwoke-secureblue/actions/runs/33298098294) (WAL-only) green. Restore bake [33302539888](https://github.com/SeRgi270710267/unwoke-secureblue/actions/runs/33302539888) (`6f22bd9`) green on compose, **red on Origin USB**.
...
**Next Origin USB test:** after this overlay bake is green, dispatch `unwoke-silverblue`. If **green**: dispatch the other three Origin flavors. If **initramfs still red**: hook must `exit 1` before titanoboa dnf; read `restored pre-flavor rpmdb` / `rpm -q kernel-core already ok` vs `FAIL: rpm -q kernel-core still failed`. A full index should install ~1 package, not 177.

**This overlay:** skip `.sqlite` / `.rpmdb-pre-flavor*` in `mark-check.py` (Trivalent compose had stamped the bak). Optional later: skip `selinux-policy-devel` dnf (prebuilt `.pp` / CIL) so Origin extra dnf stops malforming Packages in the first place.
...
- Close-chat HEAD after this file is pushed: `git log -1 --oneline`. Product overlay in GHCR `:latest` is still restore bake `6f22bd9` / run [33302539888](https://github.com/SeRgi270710267/unwoke-secureblue/actions/runs/33302539888) until the WAL-set restore bake is green.
- **In flight:** overlay bake of the WAL-set Origin restore (this commit). Do not cancel. Origin USB [33304494809](https://github.com/SeRgi270710267/unwoke-secureblue/actions/runs/33304494809) is **red** — do not re-dispatch ISO until the new GHCR `:latest` exists.
