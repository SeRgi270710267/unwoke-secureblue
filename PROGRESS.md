...
**Next Origin USB test:** after the checkpoint-before-save overlay bake is green, dispatch `unwoke-silverblue`. If **green**: dispatch the other three Origin flavors. If hook still `FAIL: rpm -q kernel-core still failed`: the ostree RPM index is unusable even after checkpoint; next is skip Origin extra dnf (prebuilt `.pp`, rpm2cpio Brave) instead of restoring a copy.

57. **ISO wrap 33305773882 (Origin silverblue, overlay `83a8dec`) FAILED in the hook.** Same 95 MiB malformed sqlite (`SELECT ... FROM 'Name'`). WAL-set restore did not help — the **saved** copy was already torn. Fail-fast worked (no 177-package titanoboa dnf). Next overlay: checkpoint WAL **before** saving the bak (Trivalent’s working end-of-compose path), drop sidecars, copy sqlite only.

**This overlay:** checkpoint-before-save in `apply-unwoke.sh`. Skip `.sqlite` / `.rpmdb-pre-flavor*` in `mark-check.py`. Optional later: skip `selinux-policy-devel` dnf (prebuilt `.pp` / CIL) so Origin extra dnf stops malforming Packages in the first place.
...
- Close-chat HEAD after this file is pushed: `git log -1 --oneline`. Product overlay in GHCR `:latest` is WAL-set bake `83a8dec` / run [33305148886](https://github.com/SeRgi270710267/unwoke-secureblue/actions/runs/33305148886) until the checkpoint-before-save bake is green.
- **In flight:** overlay bake of checkpoint-before-save (this commit). Do not cancel. Origin USB [33304494809](https://github.com/SeRgi270710267/unwoke-secureblue/actions/runs/33304494809) and [33305773882](https://github.com/SeRgi270710267/unwoke-secureblue/actions/runs/33305773882) are **red** — do not re-dispatch ISO until the new GHCR `:latest` exists.
