...
- Do not exec stock `harden_flatpak.py`. Unwoke uses `harden-flatpak.sh`. Snapshots of `harden_flatpak.py` + `flatpak.just` are watched in a job that does **not** block the overlay (kernel still ships). Drift fails that job and uploads `upstream-drift/`. Refresh: `bash .github/scripts/sync-upstream-snapshots.sh`, then port behavior into Unwoke scripts if the diff is a real improvement.
