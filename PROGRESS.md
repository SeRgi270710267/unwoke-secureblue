...
52. **Origin USB wrap:** all four Origin ISO wraps failed in titanoboa `initramfs` (`dnf install -y dracut-live`) with a malformed RPM sqlite (`SELECT hnum, blob FROM 'Packages': 11`) so `$releasever` never expanded. Trivalent + browserless wraps were green. Cause is extra Origin dnf (Brave RPM + `selinux-policy-devel` install/remove), already noted as `rpm -q` unusable. Fix: `rebuild-rpmdb.sh` last on every flavor; inspect fails the bake if sqlite is not `ok` or a WAL ships. Do **not** bump titanoboa. `#7` closed because a later Trivalent wrap was green — reused iso-alarm; Origin was still red. Alarm body now names the image.

Image-side theme, privacy.sh, Setup fingerprint button, and the rpmdb rebuild land on the **image rebuild**, not Pages. `iso.yml` / `issue-alarm.sh` are in `build.yml` paths-ignore (alarm copy does not bake). Overlay scripts (`rebuild-rpmdb.sh`, `inspect-flavor.sh`) **do** bake.
...
- This pickup (2026-08-30): cloned/pulled `a0f980e`. Overlay [84](https://github.com/SeRgi270710267/unwoke-secureblue/actions/runs/33270781562) and [86](https://github.com/SeRgi270710267/unwoke-secureblue/actions/runs/33271511130) both **green**. Scheduled [87](https://github.com/SeRgi270710267/unwoke-secureblue/actions/runs/33278659917) green. Verify 14 green. No open PRs. No open issues.
- USB ISOs from the 12-flavor dispatch: **8 green** (all Trivalent + all browserless). **4 Origin red** — titanoboa `dnf install dracut-live` on a malformed RPM db, not a pin problem. Do not bump titanoboa. Item 52 is the repair; **re-dispatch the four Origin images only after the next overlay bake is green**. Wrap uses GHCR `:latest` at pull time.
  ...
  - red Origin: [silverblue](https://github.com/SeRgi270710267/unwoke-secureblue/actions/runs/33271602415) [silverblue-nvidia-open](https://github.com/SeRgi270710267/unwoke-secureblue/actions/runs/33271603810) [kinoite](https://github.com/SeRgi270710267/unwoke-secureblue/actions/runs/33271605294) [kinoite-nvidia-open](https://github.com/SeRgi270710267/unwoke-secureblue/actions/runs/33271606614)
...
- Public mark is automatic (`mark-check.py --apply` at compose). Do not put `UNWOKE-SHIPPED-FIRST` into live Chromium/Brave/Trivalent `policies/managed`. Do not weaken privacy/security for credit.
