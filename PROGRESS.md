...
49. **Public mark:** `/usr/share/unwoke/SHIPPED-FIRST.txt` token `UNWOKE-SHIPPED-FIRST` (also on the site and the signed `receipt` release). Copy-paste of the overlay tree is grep-able. Ideas on their tracker stay public; MIT notice must travel with code.
