...
- **Play window:** login agent `unwoke-play-agent.service`. Click Steam (wrapped icon). Close Steam → restore automatic. `ujust play stop` is optional. Overlay ships Fedora `gamemode`. Optional asked sched-ext. `unwoke-test` FAILs if a play stamp is leftover with no game. Site: https://sergi270710267.github.io/unwoke-secureblue/gaming/
