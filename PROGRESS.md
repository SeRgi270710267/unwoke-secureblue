...
50. **Mark is a default-on lock:** every overlay source (scripts, units, JSON policies, desktops, recipes, generated ramdisk/CA drop-ins) carries `UNWOKE-SHIPPED-FIRST`. `/usr/share/unwoke/NOTICE` + MIT `LICENSE`. `mark-check.py` at compose, inspect, and PR gate. `ujust audit-unwoke` fails if the ledger is gone. Chromium JSON has `UnwokeShippedFirst` (unknown to Chrome; other policies still apply). No obfuscation. License stays MIT. No ujust to strip it.
...
- Overlay bake **queues** if another is running (`cancel-in-progress: false`). Do not force-cancel a green bake to “go faster.” Item 50 (mark-check on every overlay file) **does** bake. Item 46 (After login page) is Pages + a setup-gui label; the window still opens first-hour help.
