...
- Factory alarms (one reused issue per lane, not auto-merge): `factory-alarm` (canary/watch/bake/inspect), `iso-alarm` (USB wrap), `pages-alarm` (site), `vendor-installers` (contracts after allowlisted heal). Shared opener: `.github/scripts/issue-alarm.sh`. A canary hit, new `keys/secureblue.pub`, new vendor hostname, Flathub, `gpgcheck=0`, or titanoboa pin bump is **not** auto-merged. Map: `docs/factory/`.
...
32. **Mirrored stock snippets that differ here get an Unwoke box:** Flathub unfiltered, install-vpn, rebase-secureblue, bluetooth-modules, webcam-modules, Avahi unmask, brew, distrobox, audit-secureblue, create-admin, Trivalent flags. Stock commands that still apply (enroll key, kargs, USBGuard) are left alone. Does not auto-unlock.
