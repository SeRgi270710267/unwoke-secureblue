...
- **Play window:** `ujust play steam` / Setup → Gaming. Overlay ships Fedora `gamemode`. Session GameMode (+ optional asked sched-ext). Stop/crash/reboot restores. `unwoke-test` FAILs if the play stamp is leftover with no game. Does **not** swap CachyOS kernel, SMT, or mitigations. Site sell tab: https://sergi270710267.github.io/unwoke-secureblue/gaming/
