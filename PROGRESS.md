...
- GitHub Release tag `receipt` is a verification pack only (`docs/_tools/publish-receipt.sh` on full `verify.yml`). Assets: `cosign.pub` + `DIGESTS.txt` (crane digest + Unwoke cosign, no docker pull). One moving tag, rewritten in place. Never the ISO (2 GiB cap). Never fails inspect (script always exits 0). 0 verified images → leave last good receipt + alarm. Partial list → publish what verified + alarm. Next full green closes. Ignore GitHub’s source zip.
...
43. **Receipt release:** one moving GitHub tag `receipt` (pubkey + verified digests). Hands-off rewrite on full verify. `receipt-alarm` if it cannot write; last good files stay. Not the OS, not the USB.
